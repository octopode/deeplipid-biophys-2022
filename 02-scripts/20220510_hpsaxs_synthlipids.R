...
# locate data files
dir_head = here("03-mainfigs", "synsim")
pat_dat = "\\.dat$"
...
data_saxs = files_data %>% 
  ...
  load_hp_saxsdat(clust = 4L) %>% 
  # group by run
  group_by(datetime, pdir, press, temp, rep) %>% 
  mutate(
    rep  = as.numeric(rep),
    comp = composition[samp],
    comp = comp %>% factor(levels = unique(composition))
  ) %>% 
  group_by(samp, temp) %>% 
...
data_saxs_norm %>%
  ...
  #filter(comp == "18:0/20:4 PE-PPE 50-50") %>% 
  #filter(ser == 8) %>% 
...
phranges = read_tsv(here("03-mainfigs", "synsim", "20220503_phaseranges.tsv")) %>% 
  mutate(comp = comp %>% factor(levels = unique(composition)))

# extract pure phase exemplars from these data
phprofs  = data_saxs_norm %>% 
  left_join(
    phranges %>% 
      # "pure" phases only
      filter(hii + la + lb == 1), 
...
data_saxs_frac = data_saxs_norm %>% 
  ...
  group_by(ser, temp, comp, press, pdir, filename) %>% # include pdir or no?
  mutate(
  ...
  # I don't know why filename grouping is lost
  group_by(ser, temp, comp, press, filename) %>% 
  mutate(
...
data_saxs_mods = bind_rows(
  data_saxs_mods %>% filter(!((logreg == 1) & (ph == "lb") & (comp == "18:0/20:4 PPE"))),
  constr_mod
)

# run out the fits
domain_fit = seq(0, 2000, 10)
data_saxs_fits = data_saxs_mods %>% 
  group_by(ser, comp, ph, logreg) %>% 
  filter(
    !((logreg == 2) & (ph == "lb")) &
      !((logreg == 1) & (ph == "hii"))
...
#TEST sanity check!
# NTS 20220511 not sure the offset() thing is a done deal; asymmetry would be nice to fix
data_saxs_fits %>% 
  ggplot(
...
ggsave(here("03-mainfigs", "synsim", "20220511_panelC.pdf"), width=8, height=4)
...
# 20220618 PUFA raw data plot for Itay
data_saxs %>% 
  filter(ser %in% c(1, 10)) %>% 
  filter(pdir == "up") %>% 
  filter(press %in% c(0, 500, 1000, 1500, 2000)) %>% 
  filter(q <= 0.5) %>% 
  ggplot(
    aes(
      x = q,
  ...
  labs(
    x = "q (1/Å)",
    y = "log(i(q)) (arb.)",
    title = "Scattering profile vs. pressure: PE and PPE"
  )
ggsave(here("04-suppfigs", "20220618_PUFA_rawprof.pdf"), width=6, height=8)
