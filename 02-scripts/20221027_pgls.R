...
source(here("02-scripts", "20220502_lipidomics.R"))

# load the phylogeny
file_tree = here("01-rawdata", "20220506_iq.tree")
...
ggsave(here("03-mainfigs", "ctenolipids", "20220609_ols_3.pdf"), width=3, height=6)
...
# 20221027: NOW, do it all again without Tjalfie and see how it looks

# prep the phenotypic data in long form
# this can handle multiple variables
# moving forward, there are just "pred" and "resp" variables
# all else is handled by dplyr grouping
phenodata_tjalfree = bind_rows(
  # composition data
  lcmsdata_pl_wholectenos %>% 
    filter(sp != "Tjal_pink") %>% 
    filter(class %in% major_pls) %>% 
...
# Reduce the phylo- and phenodata to their taxonomic intersection
# give warnings if species in phenodata are not found in phylogeny
phenophylo_tjalfree = match_tree(phenodata_tjalfree, phylodata, tip2sp = function(x){str_remove_all(x, "[0-9]")})
pheno_tjalfree = phenophylo_tjalfree$pheno
phylo_tjalfree = phenophylo_tjalfree$phylo

# root it?
# this seems to be the conservative choice - LBA affects PGLS!
message("midpoint-rooting phylogeny")
phylo_tjalfree = phylo_tjalfree %>% midpoint.root()

# ultrametrize?
message("making phylogeny ultrametric")
phylo_tjalfree = phylo_tjalfree %>% chronos()
...
# Expand the tree so it has a tip for every individual (distinguished by trailing numbers)
# the pheno input grouping is essential!
phenophylo_indl_tjalfree = expand_tree(pheno_tjalfree %>% group_by(sp, eid), phylo_tjalfree)
pheno_indl_tjalfree = phenophylo_indl_tjalfree$pheno
phylo_indl_tjalfree = phenophylo_indl_tjalfree$phylo

# Does the tree of individuals look right?
plot(phylo_indl_tjalfree)

# now we are ready to run regressions!
pgls_mods_tjalfree = pheno_indl_tjalfree %>% 
  # for the OU model, we are gonna try a range of different gamma values, i.e. branch length multipliers
  full_join(tibble(gamma = lapply(seq(0, 20, 1), function(x){10**x}) %>% unlist()), by = character()) %>% 
  group_by(pred_var, resp_var, class, gamma) %>% 
  summarise(
...
# extract all the params nicely
pgls_results_tjalfree = pgls_mods_tjalfree %>% 
  rowwise() %>% 
  mutate(
    par_ou = mod_ou %>% 
      unpack_gls() %>% 
...
# find the best fits
params_best_tjalfree = pgls_results_tjalfree %>%
  drop_na(`logLik`) %>% 
  group_by(pred_var, resp_var, class) %>% 
  dplyr::rename(alpha = modelStruct.corStruct) %>% 
  # can adjust the fit selection criteria here
...
# calculate fits frames for each model across the domain
domain_fit = seq(0, 4000, 10)
fits_best_tjalfree = params_best_tjalfree %>% 
  rowwise() %>% 
  mutate(
    pred = list(domain_fit),
    resp = predictSE.gls(mod_ou, newdata = list("pred" = unlist(pred))) %>% list()
...
phenodata_tjalfree %>% 
  ...
  # underlay precalculated fits, robust to transformation unlike abline
  geom_ribbon(
    data = fits_best_tjalfree %>% filter(resp_var %in% subs_resp),
    aes(
      ymin = resp - resp_serr,
      ymax = resp + resp_serr
    ),
  ...
  #coord_trans(x = scales::exp_trans(10)) +
  ggtitle("PGLS: [PPE] and PL curvature vs. depth")
ggsave(here("03-mainfigs", "ctenolipids", "20221027_pgls_tjalfree.pdf"), width=6, height=8)
...
geom_smooth(
  ...
  guides(fill  = "none") +
  ggtitle("OLS: [PPE] and [PE]\nvs. depth")
ggsave(here("03-mainfigs", "ctenolipids", "20221027_ols_tjalfree.pdf"), width=3, height=6)

# rn need to check significance with e.g.:
params_best_tjalfree %>% .$mod_ou %>% lapply(., summary) %>% setNames(params_best$resp_var)
