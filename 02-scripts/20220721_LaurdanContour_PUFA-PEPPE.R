...
pat_data = ".*JWL023[58].*.tsv$" # filename pattern
...
raw2fluor = function(rawdata, lop = 2){
  rawdata %>% 
    filter(shutter) %>% 
    separate(msg, sep="_", into=c("tdir", "pdir", "rep", "cyc")) %>% 
    group_by(samp, state, wl_ex, wl_em, tdir, pdir) %>% 
    arrange(watch) %>% 
    # drop the first reading(s)
    filter(row_number() > lop) %>% 
    mutate(n = n()) %>% 
    group_by(samp, state, tdir, pdir, wl_ex, wl_em, P_set, T_set, n) %>% 
...
fl2gp = function(fluordata){
  fluordata %>% 
    group_by(samp, state, wl_ex, tdir, pdir) %>% 
    #group_by(across(c(-intensity_mean, -intensity_se, -wl_em))) %>%
    arrange(wl_em) %>% 
    mutate(across(contains("_act_mean"), mean)) %>% 
    group_by(across(contains("_act_mean")), samp, state, wl_ex, tdir, pdir) %>% 
    summarize(
...
# With directly labeled contours (which have to be masked in post)
scape_gp %>%
  ungroup() %>% 
  arrange(desc(samp)) %>% 
  mutate(samp = samp %>% factor(., levels=unique(.))) %>%
  drop_na(gp) %>% 
...
ggsave(here("03-mainfigs", "synsim", "20220724_LabeledContours_2.pdf"), width=4, height=6)


# Generic slide version
scape_gp %>% 
  ...
  ggtitle("C-laurdan GP")
ggsave(here("05-slides", "20220722_PUFA-PEPPE_claurdan.pdf"), width=12, height=4.75)

# line plots vs. pressure and temperature
# vs. pressure
# which temperature stop?
temp_sec = 5
# and the tolerance around it
temp_tol = 0.5

data_gp %>% 
  # filtering the raw data
  filter(wl_ex == 340) %>% 
  filter(between(T_act_mean, temp_sec-temp_tol, temp_sec+temp_tol)) %>% 
  ggplot(
    aes(
      x = gp,
      y = P_act_mean,
  ...
  geom_line(
    data = scape_gp %>% 
      filter(T_act_mean == temp_sec)
  ) +
  geom_point(
    aes(shape = pdir)
  ) +
  scale_y_reverse() +
  scale_shape_manual(values = c("pdir:dn" = 24, "pdir:up" = 25)) +
  scale_color_manual(values = c("JWL0235" = "#BAE4B3", "JWL0238" = "#BBBDBF")) +
  theme_pubr() +
  labs(
    x = "C-Laurdan GP",
    y = "Experimental pressure (bar)"
  ...
  theme(legend.position = "none")
ggsave(here("03-mainfigs", "synsim", "20220722_PUFA-PEPPE_claurdan_Psection_5C.pdf"), width=4, height=3)
...
data_gp %>% 
  # filtering the raw data
  filter(wl_ex == 340) %>% 
  filter(between(P_act_mean, press_sec-press_tol, press_sec+temp_tol)) %>% 
  ggplot(
    aes(
      x = T_act_mean,
      y = gp,
  ...
  geom_line(
    data = scape_gp %>% 
      # get nearest interp pt.
      mutate(diff = abs(P_act_mean - press_sec)) %>% 
  ...
  theme(legend.position = "none")
ggsave(here("04-suppfigs", "S7-LaurdanInversion", "20220724_PUFA-PEPPE_claurdan_Tsection_500bar_blkpts.pdf"), width=4, height=2)

# Temperature sections at multiple depths
press_sec = c(0.7, 500)
press_tol = 0.5

data_gp %>% 
  mutate(samp = ifelse(samp == "JWL0235", "XWL0235", samp)) %>% 
  # filtering the raw data
  filter(wl_ex == 340) %>% 
  full_join(tibble(sec = press_sec), by=character()) %>% 
  mutate(diff = abs(P_act_mean - sec)) %>% 
  filter(diff <= press_tol) %>% 
  ggplot(
    aes(
      x = T_act_mean,
      y = gp,
      color = samp,
      fill = samp,
      group = paste(sec, samp),
      linetype = as.factor(sec)
    )
  ) +
  facet_grid(rows = vars(samp), scales = "free_y") +
  geom_point(
    aes(
      shape = pdir,
      alpha = sec
  ...
  scale_shape_manual(values = c("pdir:dn" = 24, "pdir:up" = 25)) +
  scale_color_manual(values = c("XWL0235" = "#BAE4B3", "JWL0238" = "#BBBDBF")) +
  scale_fill_manual(values = c("XWL0235" = "#BAE4B3", "JWL0238" = "#BBBDBF")) +
  ...
  theme(legend.position = "none")
ggsave(here("04-suppfigs", "S7-LaurdanInversion", "20220724_PUFA-PEPPE_claurdan_Tsection_0-500bar_blkpts.pdf"), width=4, height=3)
...
data_gp %>% 
  ...
  ggplot(
    aes(
      ...
      fill = samp,
      group = paste(sec, samp),
      linetype = as.factor(sec) %>% fct_rev()
    )
  ) +
  facet_grid(rows = vars(samp), scales = "free_y") +
  geom_point(
  ...
  theme(legend.position = "none")
ggsave(here("04-suppfigs", "S7-LaurdanInversion", "20220724_PUFA-PEPPE_claurdan_Tsection_0-500bar_blkgray.pdf"), width=4, height=3)
