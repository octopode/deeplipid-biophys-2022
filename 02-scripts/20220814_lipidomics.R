...
file_meta = here("01-rawdata", "20220816_metadata.tsv")
...
# HG classes averaged by species
# Selected spp for Fig. S2E
lcmsdata_pl_wholectenos %>%
  filter(sp %in% spp_figS2E) %>% # comes from lipidomics_helpers
  mutate(sp = factor(sp, levels = spp_figS2E)) %>% 
  group_by(sp, sp_eid, class, depth_col, temp_col) %>%
  summarize(frac_molar = sum(frac_molar)) %>% 
  # average
  group_by(sp, class) %>% 
  summarize(
    frac_molar = mean(frac_molar),
    n = sp_eid %>% unique() %>% length(),
    minz = min(round(depth_col)),
    maxz = max(round(depth_col)),
    mint = min(round(temp_col, 1)),
    maxt = max(round(temp_col, 1))
  ) %>% 
  # and renorm
  group_by(sp) %>% 
  mutate(frac_molar = frac_molar/sum(frac_molar)) %>% 
...
ggsave(here("04-suppfigs", "20220814_FigS2E.pdf"), width=6, height=4)
...
lcmsdata_pl %>% 
  ...
  facet_wrap(~sid, scales = "free_x", ncol = 4) +
  # draw little hairlines in between compounds
  geom_col(width = 0.95, size = 0.05, color = "white") +
  geom_text(
    aes(
...
ggsave(here("04-suppfigs", "20220814_Tjalfie-tentacles.pdf"), width=6, height=4)
...
#files_metadata = list.files(here("01-rawdata"), "metadata")

# get unique dive numbers
lcmsdata_pl_wholectenos %>% 
  distinct(sid) %>% 
  filter(str_starts(sid, c('[VDM]'))) %>% 
  separate(sid, c("dive", "samp")) %>% 
  .$dive %>% 
  paste(., collapse=',')

# Table S1 and raw lipidomics data file
allsamps_wide_metadat = lcmsdata_pl_wholectenos %>% 
  # idk what order the compounds are in but it makes no sense
  # this is satisfactory
  arrange(class, carbon, dbonds, id) %>% 
  mutate(id = id %>% factor(., levels = unique(.))) %>% 
  arrange(
    -shal, 
    cold,
    -temp_col_spmean,
  ...
  select(eid, sid, sp, id, frac_molar, depth_col, temp_col) %>% 
  group_by(eid, sid, sp, depth_col, temp_col) %>% 
  pivot_wider(names_from = "id", values_from = "frac_molar") %>% 
  # join to lat/lon
  left_join(
    bind_rows(
      read_tsv("/Users/jwinnikoff/Documents/MBARI/Lipids/cteno-lipids-2020/00-metadata/20201025_lipids_metadata_docr_vnta_bw.tsv") %>% rename(sid=samplenum),
      read_tsv("/Users/jwinnikoff/Documents/MBARI/Lipids/cteno-lipids-2020/00-metadata/20201025_lipids_metadata_goc_kok_kmt.tsv") %>% rename(sid=samplenum),
      read_tsv("/Users/jwinnikoff/Documents/MBARI/Lipids/cteno-lipids-2020/00-metadata/20201025_lipids_metadata_seb_sva_grl_icl.tsv") %>% rename(sid=samplenum) %>% select(-datetime)
    ) %>% 
      select(sid, lat, lon),
    by = "sid"
  ) %>% 
  # fill in the blanks using additional retrieved dive data
  separate(sid, into = c("divenum", "sampnum"), sep='-', remove = FALSE) %>% 
  left_join(
    read_tsv(here("01-rawdata", "20220823_ROVcoords.tsv")) %>% 
      mutate(divenum = str_to_upper(str_sub(RovName, end=1)) %>% paste(DiveNumber, sep='')) %>% 
      select(divenum, lat, lon),
    by = "divenum"
  ) %>% 
  # reconcile lat/lon
  rowwise() %>% 
  mutate(
    lat = mean(c(lat.x, lat.y), na.rm = TRUE),
    lon = mean(c(lon.x, lon.y), na.rm = TRUE)
  ) %>% 
  select(-contains("x|y"))
# that leaves us without coords for just a couple KM samples and some Arctic samples

# save the big gnarly raw data table
allsamps_wide_metadat %>% write_tsv(here("04-suppfigs", "20220823_rawlipidomics.tsv"))

# save the metadata-only Table S1
allsamps_metadat_only = allsamps_wide_metadat %>% 
  select(lat, lon) %>% 
  write_tsv(here("04-suppfigs", "20220823_justmetadata.tsv"))
