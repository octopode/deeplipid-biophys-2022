...
lcmsdata_long = lcmsdata_wide %>%
  ...
  parse_lipidmaps_id() %>%
  # correct for fact that DAG, TAG stds are 10X of PLs
  group_by(id) %>%
...
ggsave(here("04-suppfigs", "20220517_all_phospholipids.pdf"), width=11, height=7.5)
...
ggsave(here("04-suppfigs", "20220502_all_carbon.pdf"), width=10, height=7.5)
...
ggsave(here("04-suppfigs", "20220524_orig_carbon.pdf"), width=10, height=7.5)
...
ggsave(here("04-suppfigs", "20220502_all_dbonds.pdf"), width=10, height=7.5)
...
lcmsdata_pl_wholectenos %>% filter(eid %in% eids_orig) %>% mutate(id = as.character(id)) %>% group_by(id) %>% summarise()
